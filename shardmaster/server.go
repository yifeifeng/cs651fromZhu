...
func (sm *ShardMaster) WaitForApply(index int) (Result, bool) {
	sm.mu.Lock()
	if _, ok := sm.replych[index]; !ok {
		sm.replych[index] = make(chan Result, 1)
	}
	resultChan := sm.replych[index]
	sm.mu.Unlock()
...
func (sm *ShardMaster) Init() {
	...
	sm.replych = make(map[int]chan Result,1)
}

func (sm *ShardMaster) CheckSeqNum(clientId int64, reqId int) bool {
	sm.mu.Lock()
	defer sm.mu.Unlock()

	if last, ok := sm.lastseq[clientId]; ok && last >= reqId {
		return true
	}
	sm.lastseq[clientId] = reqId
	return false
}
...
func (sm *ShardMaster) SendResult(index int, result Result) {
	...
	if _, ok := sm.replych[index]; !ok {
		...
		select {
		case <-sm.replych[index]:
		default:
		}
	}

	sm.replych[index] <- result
}
...
func (sm *ShardMaster) Loop() {
	for {
		...
		result.Reply = sm.ApplyOp(req, sm.CheckSeqNum(clientId, reqId))
